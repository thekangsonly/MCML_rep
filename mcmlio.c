...
static short next_token(const char** start, const char** end)
{
	if (!*end)					   // first call
		*end = *start;
	...
	while (**end && **end != '\t') // advance to terminating zero or next tab
		++* end;
...
/***********************************************************
 *	Skip space or comment lines and return a data line only.
 *	Ignore any trailing chars after '#' token.
 ****/
void NewFindDataLine(FILE* File_Ptr, char* buf)
{
	buf[0] = '\0';
	do {	/* skip space or comment lines. */
		if (fgets(buf, 255, File_Ptr) == NULL) {
...
void ReadFnameFormat(FILE* File_Ptr, InputStruct* In_Ptr)
{
	...
	if (buf[0] == '\0') {
		nrerror((char*)"Reading file name and format.\n");
		sscanf(buf, "%s %c", In_Ptr->out_fname, &(In_Ptr->out_fformat));
	}

	token = strtok(buf, "\t");
	// deep copy: copy the whole token string as out_fname is an array of char.
	strcpy(In_Ptr->out_fname, token);
	token = strtok(NULL, buf);
	In_Ptr->out_fformat = *token;
...
{
	char buf[STRLEN];  /** read in number of photons. **/
	NewFindDataLine(File_Ptr, buf);

	if (buf[0] == '\0') {
		nrerror((char*)"Reading number of photons.\n");
		sscanf_s(buf, "%ld", &In_Ptr->num_photons);
	}

	In_Ptr->num_photons = atol(buf);
	if (In_Ptr->num_photons <= 0)
		nrerror((char*)"Nonpositive number of photons.\n");
}
...
void ReadDzDr(FILE *File_Ptr, InputStruct *In_Ptr)
{
	...
	const char* target = NULL; // NULL value indicates first call

	if (buf[0] == '\0') {
		nrerror((char*)"Reading dz, dr.\n");
		sscanf(buf, "%lf%lf", &In_Ptr->dz, &In_Ptr->dr);
	}

	In_Ptr->dz = atof(token);
	if (In_Ptr->dz <= 0)
		nrerror((char*)"Nonpositive dz.\n");

	next_token(&token, &target);
	In_Ptr->dr = atof(target);
	if (In_Ptr->dr <= 0)
		nrerror((char*)"Nonpositive dr.\n");
}
				 ...
				 char *side)
{
	...
	if (buf[0] == '\0') {
		sprintf(msg, "Reading n of %s ambient.\n", side);
		nrerror(msg);
	}

	Layer_Ptr->n = atoi(buf);
					 ...
					 double *Z_Ptr)
{
  ...
  NewFindDataLine(File_Ptr, buf);
	...
	next_token(&token, &target);
	Layer_Ptr->n = atof(token);

	next_token(&token, &target);
	Layer_Ptr->mua = atof(token);

	next_token(&token, &target);
	Layer_Ptr->mus = atof(token);

	next_token(&token, &target);
	Layer_Ptr->g = atof(token);
